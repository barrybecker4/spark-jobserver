...
class AkkaClusterSupervisorActorSpec
  ...
  override def beforeAll() {
    ...
    supervisor = system.actorOf(
      Props(classOf[StubbedAkkaClusterSupervisorActor], daoActor, TestProbe().ref,
        managerProbe, cluster, visitedContextIsFinalStatePath),
      "supervisor")
  }
  ...
  describe("Context create tests") {
    ...
    it("should be able to start multiple contexts") {
      ...
      supervisor ! ListContexts
      expectMsgAnyOf(ArrayBuffer("test-context7", "test-context8"), ArrayBuffer("test-context8", "test-context7"))
    }
     ...
     */
    it("should kill context if JVM creation timed " +
      "out already and context is trying to join/initialize after Jobserver was restarted") {
      visitedContextIsFinalStatePath.set(0)
      ...
      val finishedContextId = finishRejoiningManagerProbe.ref.path.name
        .replace(AkkaClusterSupervisorActor.MANAGER_ACTOR_PREFIX, "")
    ...
    it("should kill context if it was marked as error in DAO and it is trying to join/initialize") {
      ...
      val erroredOutContextId = erroredOutRejoiningManagerProbe.ref.path.name
        .replace(AkkaClusterSupervisorActor.MANAGER_ACTOR_PREFIX, "")
